...
extern void get_nvram_data(unsigned char *pBuf);
extern unsigned long get_nvram_start_addr(void);  //Alex 
extern void kerSysFlashInit(void);  //Alex
...
//alex add ralink_wps 2007.7.3
static unsigned char ralink_wps_enable = 0 ;
...
//Alex : add for Belkin WPS default PIN
void kerSysGetSN(char* SerialNumber) 
{
	NVRAM_DATA nvramData;
	readNvramData(&nvramData);	
	memcpy(SerialNumber , nvramData.SerialNumber , SERIAL_NUMBER_LENGTH);
}
void kerSysSetSN(char* SerialNumber) 
{
	NVRAM_DATA nvramData;
	UINT32 crc = CRC32_INIT_VALUE;	
	readNvramData(&nvramData);
	memcpy(nvramData.SerialNumber , SerialNumber , SERIAL_NUMBER_LENGTH);
    	nvramData.ulCheckSum = 0;
    	crc = getCrc32((char *)&nvramData, (UINT32) sizeof(NVRAM_DATA), crc);      
    	nvramData.ulCheckSum = crc;
...
void kerSysCountrySet(char *string, int strLen, int offset)
{
	UINT32 crc = CRC32_INIT_VALUE;
   	NVRAM_DATA NvramData;
	volatile unsigned char *mac, *tmphwaddr;
	...
	//set country
	memcpy(NvramData.chReserved, string,2);
	printk("Flash chReserved = %s\n", string);
...
void kerSysCountryGet(char *value)
{
	NVRAM_DATA nvramData;
	
	readNvramData(&nvramData);	
	memcpy(value, nvramData.chReserved, 2);
}
                        ...
                        unsigned int command, unsigned long arg )
{
    ...
    switch (command) 
    {
        	...
        	case BOARD_IOCTL_GET_SN:
            		if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0) 
            		{
              		...
              		kerSysGetSN( SerialNumber);
              		memcpy(ctrlParms.buf, &SerialNumber[0],SERIAL_NUMBER_LENGTH);
              		ctrlParms.buf[SERIAL_NUMBER_LENGTH] = '\0';                        
        	...
        	case BOARD_IOCTL_SET_SN:
	 		if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0) 
				kerSysSetSN(ctrlParms.buf);
			else				
				printk("dumas copy_from_user error\n");				
            		break;

		case BOARD_IOCTL_SET_COUNTRY:
			if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0)
				kerSysCountrySet(ctrlParms.buf,0,0);
			else
				printk("dumas copy_from_user error\n");
		break;

		case BOARD_IOCTL_GET_COUNTRY:
            		if (copy_from_user((void*)&ctrlParms,(void*)arg, sizeof(ctrlParms)) == 0) 
	            	{
				kerSysCountryGet(tmp);
				memcpy(ctrlParms.buf, tmp, 2*sizeof(char));
        	        	ctrlParms.result = 0;
            	    		__copy_to_user((BOARD_IOCTL_PARMS*)arg,&ctrlParms, sizeof(BOARD_IOCTL_PARMS));
	                	ret = 0;
    	        	}
	...
	//add by Alex for Ralink WPS
        case BOARD_IOCTL_RALINK_WPS:
		if (ralink_wps_enable)
		{
			ctrlParms.result = RALINK_WPS_ENABLE ; 
			__copy_to_user((BOARD_IOCTL_PARMS*)arg, &ctrlParms, sizeof(BOARD_IOCTL_PARMS));
		}
        break;                        
	//add end
...
{   
    if (sesBtn_pressed()){
    	 //alex add for ralink_wps 2007.7.3
    	 ralink_wps_enable = 1 ;
	 kerSysWakeupMonitorTask();  //monitor the task in user space , inform wps button is pressed.
...
EXPORT_SYMBOL(kerSysNvRamGet); //Alex
EXPORT_SYMBOL(kerSysGetSN);//Alex
EXPORT_SYMBOL(kerSysSetSN);//Alex
